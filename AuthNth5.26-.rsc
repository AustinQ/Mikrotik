...
/ip firewall mangle add action=mark-connection chain=prerouting comment=("nth_conn_".$i) connection-state=new disabled=no new-connection-mark=("conn_".$i) nth=($total,$num) passthrough=yes src-address=192.168.0.0/24;
/ip firewall mangle add action=mark-routing chain=prerouting comment=("nth_rout_".$i) connection-mark=("conn_".$i) disabled=no new-routing-mark=("rout_".$i) passthrough=yes src-address=192.168.0.0/24;

/ip firewall nat add action=masquerade chain=srcnat disabled=no out-interface=$i comment=("nth_nat_".$i) src-address=192.168.0.0/24;
